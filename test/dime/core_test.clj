...
(def ten-times (di/inj {:expose :ten-multiplier} [^{:inject :multiplier} n] (* 10 ^long n)))
...
(deftest test-assoc-inj
  ...
  (is (contains? (di/assoc-inj {} ten-times (di/inj {:expose :foo-bar} [foo bar] [foo bar])) :foo-bar)))
